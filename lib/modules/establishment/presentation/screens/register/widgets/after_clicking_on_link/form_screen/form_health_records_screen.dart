  ...
  Future<void> _fetchInitialData() async {
    try {
      final docsFuture = getHREmployeeDoc(
        context,
        FrontendConfigStore.data!.config.healthDocId,
        1,
        999,
      );
      ...
      final ackFuture = getAckHealthRecord(
        ...
        FrontendConfigStore.data!.config.healthDocId,
        widget.employeeID,
        'no', // approveOnly: pass 'yes' if only approved records should prefill
      );
      ...
      healthrecord.add(documents);
    } catch (error) {
      healthrecord.addError(error);
    }
  }
        ...
        children: [
          const Padding(
            ...
            child: FormsBlueHeaderBulletsConst(
              bullets: [
                'Instructions: This section is designed to capture and document your health and records as part of your onboarding process. Please enter the required information accurately.',
                'Please ensure that all uploaded records are clear and legible. Accepted file formats for uploads include PDF, In case of declination, please upload a signed copy of the declination form.',
              ],
            ),
          ),
            ...
            builder: (BuildContext context,
              ...
              if (snapshot.connectionState == ConnectionState.waiting) {
                ...
                if (_fileNames.length == documents.length) {
                  // Notify the progress provider once lists are in sync,
                  // same as before.
                ...
                onPressed: () async {
                  ...
                  try {
                    ...
                    if (!hasAnyFile) {
                      showDialog(
                        context: context,
                        builder: (_) => const VendorSelectNoti(message: 'Please Select File'),
                      );
                      return;
                    }
                    ...
                    for (int i = 0; i < finalPaths.length; i++) {
                      ...
                      currentBatch.add(() async {
                        final response = await uploadDocuments(
                          context: context,
                          employeeDocumentMetaId: FrontendConfigStore.data!.config.healthDocId,
                    ...
                    await showDialog(
                      context: context,
                      builder: (_) => const AddSuccessPopup(message: 'Document Uploaded Successfully.'),
                    );

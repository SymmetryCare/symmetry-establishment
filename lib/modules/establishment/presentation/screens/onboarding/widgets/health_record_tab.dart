...
class _HealthRecordConstantState extends State<HealthRecordConstant> {
  ...
  // time a checkbox toggle triggers setState — if we built a brand new
  // Future each build (even one that resolves instantly from a byte cache),
  // FutureBuilder still briefly shows the loading state for that new future
  // before it resolves, causing the visible blink.
  ...
  Future<void> _fetchData() async {
    try {
      var data = await getAckHealthRecord(
          context, FrontendConfigStore.data!.config.healthDocId, widget.employeeId, 'no');
      data.sort((a, b) {
  ...
  //   iframe, no native browser PDF-viewer chrome, so no scrollbar/toolbar
  //   to fight with; looks like a Drive/Gmail attachment preview
  // - anything else (doc/docx, unknown) falls back to the generic icon,
  //   since browsers can't render those formats inline without a
  //   server-side conversion/thumbnail endpoint
  ...
  // ── Helpers for the new card layout ──────────────────────────────────────
  ...
  // NOTE: OnboardingAckHealthData doesn't currently expose file size / uploaded
  // date. Assuming fields `fileSizeKb` (num) and `uploadedAt` (DateTime) will be
  // added to the model + manager mapping. Falls back gracefully if null.
  String _fileMetaText(OnboardingAckHealthData data) {
    String sizeText = 'File';
    try {
      final dynamic sizeKb = (data as dynamic).fileSizeKb;
      if (sizeKb != null) sizeText = '${sizeKb.toString()} KB';
    } catch (_) {}

    String dateText = '';
    try {
      final dynamic uploadedAt = (data as dynamic).uploadedAt;
      if (uploadedAt is DateTime) {
        dateText = 'Uploaded on ${DateFormat('dd MMM yyyy hh:mm a').format(uploadedAt)}';
      }
    } catch (_) {}

    return dateText.isEmpty ? sizeText : '$sizeText  •  $dateText';
  }
  ...
  Widget _buildDocumentCard(BuildContext context, OnboardingAckHealthData data, int index) {
    ...
    // Always try to render the actual uploaded file — images load directly,
    // PDFs render via a rasterized first-page bitmap, everything else falls
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          // ── Footer: size/date + view/download/delete icons ──────────────────
          Row(
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<List<OnboardingAckHealthData>>(
      ...
      builder: (context, snapshot) {
        ...
        // jumping between centerLeft (single) and spaceBetween (multiple).
        ...
        return Container(
          ...
          child: Padding(
            ...
            child: Column(
              // ✅ start instead of center — a fixed-width Container centering
              // its Column child shifts the single-card case relative to
              // where card #1 sits with multiple cards. start keeps it fixed.
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                      ...
                      Flexible(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            Text(
                              'Review candidate health related documents. Click on any document to preview, zoom or download.',
                              softWrap: false,
                              style: CustomTextStylesCommon.commonStyle(
                        ...
                        children: [
                          ...
                          ElevatedButton(
                            ...
                            style: ElevatedButton.styleFrom(
                              ...
                              foregroundColor: hasSelection ? Colors.white : ColorManager.mediumgrey,
                              shape: RoundedRectangleBorder(
                                borderRadius: BorderRadius.circular(8),
                              ),
                            ),

...
class QualificationEmployment extends StatefulWidget {
  ...
  const QualificationEmployment({required this.employeeId});

  @override
  State<QualificationEmployment> createState() =>
      _QualificationEmploymentState();
}
...
class _QualificationEmploymentState extends State<QualificationEmployment> {
  ...
  // tabs): replaced _valueText with _adaptiveValue. Overflow is decided by
  // measuring the actual text against the real available column width
  // using TextPainter — hover (MouseRegion + CompositedTransformTarget) is
  // only wired up when the text truly overflows and gets clipped with
  // "...". If it fits, it renders as plain text with no hover overhead.
  //
  // ASSUMPTION: HrOnboardingProvider exposes showOverlay/removeOverlay
  // with the same signature as HrManageProvider (used in the
  // Employment/Education/Licenses/References tabs). If it doesn't yet,
  // add matching methods to HrOnboardingProvider — the calls below expect
  // provider.showOverlay(context, Offset, String) and
  ...
  Widget build(BuildContext context) {
    final providerEmpState =
    Provider.of<HrOnboardingProvider>(context, listen: false);
    WidgetsBinding.instance.addPostFrameCallback((_) {
      providerEmpState.getEmployeeData(context, widget.employeeId);
    });
                            ...
                            children: List.generate(snapshot.data!.length,
                                  ...
                                  return CardDetails(
                                      childWidget: DetailsFormate(
                                        ...
                                        // FIX: always a fixed-height widget now,
                                        // never null, so the title row height
                                        // stays constant with or without a
                                        // document to view.
                                        ...
                                        row1Child2: [
                                          ...
                                          _adaptiveValue(
                                            context,
                                            fullText: snapshot.data![index].dateOfJoin,
                                            link: _linkFor(index, 'dateOfJoin'),
                                            provider: providerState,
                                          ),
                                          _adaptiveValue(
                                            context,
                                        ...
                                        row2Child2: [
                                          ...
                                          _adaptiveValue(
                                            context,
                                            fullText: snapshot.data![index].supervisor?.toString(),
                                            link: _linkFor(index, 'supervisor'),
                                            provider: providerState,
                                          ),
                                          _adaptiveValue(
                                            context,
                                            fullText: snapshot.data![index].supMobile?.toString(),
                                        ...
                                        // fixed height + right-aligned so the
                                        // "Approved"/"Rejected" text state and
                                        // the Reject/Approve buttons state
                                        // both occupy the same outer slot,
                                        // regardless of QualificationActionButtons'
                                                  ...
                                                  onRejectPressed: () async {
                                                    showDialog(
                                                      ...
                                                      builder:
                                                        ...
                                                        return RejectDialog(
                                                          onYesPressed: () async {
                                                            await rejectOnboardQualifyEmploymentPatch(
                                                                context,
                                                                snapshot.data![index]
                                                                    .employmentId);
                                                            providerState

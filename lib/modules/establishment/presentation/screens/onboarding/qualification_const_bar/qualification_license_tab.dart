...
class QualificationLicense extends StatefulWidget {
  ...
  const QualificationLicense({Key? key, required this.employeeId})
      : super(key: key);
...
class _QualificationLicenseState extends State<QualificationLicense> {
  ...
  // tabs, and QualificationEmployment/QualificationEducation): replaced
  // _valueText with _adaptiveValue. Overflow is decided by measuring the
  // actual text against the real available column width using
  // TextPainter — hover (MouseRegion + CompositedTransformTarget) is only
  // wired up when the text truly overflows and gets clipped with "...".
  ...
  Widget build(BuildContext context) {
    final providerLicenseState =
    Provider.of<HrOnboardingProvider>(context, listen: false);
    WidgetsBinding.instance.addPostFrameCallback((_) {
      providerLicenseState.getLicenseData(context, widget.employeeId);
    });
                                    ...
                                    return CardDetails(
                                        childWidget: DetailsFormate(
                                              ...
                                              child: documentUrl == "--"
                                                ...
                                                onPressed: () {
                                                  downloadFile(context: context,
                                                      fileUrl: documentUrl!,
                                                      documentName:"license${index + 1}_document.pdf",
                                          ...
                                          row1Child2: [
                                            ...
                                            _adaptiveValue(
                                              context,
                                              fullText: snapshot.data![index].licensure,
                                              link: _linkFor(index, 'licensure'),
                                          ...
                                          row2Child2: [
                                            ...
                                            _adaptiveValue(
                                              context,
                                              fullText: snapshot.data![index].expDate,
                                              link: _linkFor(index, 'expDate'),
                                              provider: providerState,
                                            ),
                                          ],
